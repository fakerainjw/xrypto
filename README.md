...
단위 테스트는 응답 *형태*를 가정한다. 그 가정이 실제와 맞는지는 러너에서 확인한다.
...
`scripts/live_smoke.py`가 실제 업비트 API를 때려서 어느 엔드포인트가 선택되는지,
응답 필드가 저장 스키마와 맞는지, `to`가 정말 exclusive인지, `Remaining-Req`에
`sec`가 오는지, 두 번째 실행이 증분으로만 도는지를 검증한다. 원본 응답은
`fixtures/` 아티팩트로 올라오므로, 받아서 회귀 테스트 픽스처로 쓸 수 있다.

러너는 해외 IP라 국내 엔드포인트가 막히는지도 여기서 실제로 드러난다.
